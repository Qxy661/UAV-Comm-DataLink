...
> **相关文档：** [5G-NR 与 URLLC](./03-5G-NR与URLLC.md)
...
### 2.4 覆盖仿真（3GPP 路径损耗模型）

以下代码基于 3GPP TR 36.777 简化模型，仿真不同飞行高度下 UAV 的路径损耗与 LoS 概率：

```python
import numpy as np
import matplotlib.pyplot as plt

def cell_coverage_model(d_2d, h_uav, h_bs=25, freq_GHz=2.0):
    """3GPP TR 36.777 path loss model for UAV
    
    Args:
        d_2d: 水平距离 (m)
        h_uav: 无人机飞行高度 (m)
        h_bs: 基站天线高度 (m), 默认 25m
        freq_GHz: 载波频率 (GHz), 默认 2.0 GHz
    Returns:
        pl: 路径损耗 (dB)
        p_los: 视距概率
    """
    d_3d = np.sqrt(d_2d**2 + (h_uav - h_bs)**2)
    # LoS probability (elevation-angle dependent)
    theta = np.arctan2(h_uav - h_bs, d_2d) * 180 / np.pi
    p_los = 1 / (1 + 10 * np.exp(-0.6 * (theta - 10)))
    # Path loss (simplified 3GPP model)
    pl_los = 28 + 22 * np.log10(d_3d) + 20 * np.log10(freq_GHz)
    pl_nlos = 13.54 + 39.08 * np.log10(d_3d) + 20 * np.log10(freq_GHz) - 0.6 * (h_uav - 1.5)
    pl = p_los * pl_los + (1 - p_los) * pl_nlos
    return pl, p_los

# Plot coverage vs altitude
...
for h in altitudes:
    pl, p_los = cell_coverage_model(d_range, h)
    ax1.plot(d_range / 1000, pl, label=f'h={h}m')
    ax2.plot(d_range / 1000, p_los, label=f'h={h}m')

ax1.set_xlabel('Distance (km)')
ax1.set_ylabel('Path Loss (dB)')
ax1.set_title('UAV Path Loss vs Altitude (3GPP Model)')
ax1.legend()
ax1.grid(True)

ax2.set_xlabel('Distance (km)')
ax2.set_ylabel('LoS Probability')
ax2.set_title('LoS Probability vs Altitude')
ax2.legend()
ax2.grid(True)

plt.tight_layout()
plt.savefig('uav_coverage.png', dpi=150)
plt.show()
```

> **说明：** 该模型适用于初步仿真评估。实际部署中需结合基站天线图、地形数据和建筑物分布进行精确仿真。
